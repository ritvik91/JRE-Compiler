...
public class CodeGenVisitorTest implements ImageResources{
	...
	/*
	@Test
	public void imageGroup() throws Exception{
		devel = false;
		grade = true;
		...
		for(int y = 0; y < 1024; y++) {
			for (int x = 0; x < 1024; x++) {
				...
				int pixelRef = ImageSupport.getPixel(loggedImage`, x, y);
				int pixel = ImageSupport.getPixel(loggedImage, r,a);
	...
	@Test
	public void imageCopyRA() throws Exception{
		devel = false;
		grade = true;
		String prog = "imageCopy";
		String input = prog 
	...
	@Test
	public void imageI0() throws Exception {
	...
	public void imageGen9() throws Exception {
		devel = false;
		grade = true;
		String prog = "imageGen";
		String input = prog + "\nimage[128,128] g; \n" + "g[[r,a]] = polar_r[cart_x[polar_r[x,y],polar_a[x,y]], cart_y[polar_r[x,y],polar_a[x,y]]];" + "g -> SCREEN;\n";
		byte[] bytecode = genCode(input);
		String[] commandLineArgs = {};
		runCode(prog, bytecode, commandLineArgs);
		BufferedImage loggedImage = RuntimeLog.globalImageLog.get(0);
		for (int y = 0; y < 128; y++) {
			for (int x = 0; x < 128; x++) {				
				int pixelRef = RuntimeFunctions.polar_r(RuntimeFunctions.cart_x(RuntimeFunctions.polar_r(x, y), RuntimeFunctions.polar_a(x, y)), RuntimeFunctions.cart_y(RuntimeFunctions.polar_r(x, y), RuntimeFunctions.polar_a(x, y)));
			//Statement_Assign [lhs=name [name=g, index=Index [e0=Expression_PredefinedName [name=KW_r], e1=Expression_PredefinedName [name=KW_a]]], e=Expression_FunctionAppWithIndexArg [function=KW_polar_r, arg=Index [e0=Expression_PredefinedName [name=KW_r], e1=Expression_PredefinedName [name=KW_a]]]]	
				int pixel = ImageSupport.getPixel(loggedImage, x, y);
				assertEquals(pixelRef, pixel);				
			}
		}
